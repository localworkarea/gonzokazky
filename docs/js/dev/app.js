...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function createAnimation() {
    ...
    ScrollTrigger.create({
      ...
      onEnter() {
        shacksImg.classList.add("_active");
      }
    ...
    function buildSectionsTimelines({ start, end }) {
      sections.forEach((sec, i) => {
        ...
        const tl = gsap.timeline({
          ...
          scrollTrigger: {
            id: `secReveal_${i}`,
            // уникальный id на всякий случай
            trigger: root,
            start,
            end,
            scrub: 1,
            scroller: smoother.scrollContainer
            // важнo, ты используешь ScrollSmoother
            // markers: true,
          }
        });
        tl.to(banner, { opacity: 1 }).to(txt, { opacity: 1 }, "-=0.1").to(btnWr, { opacity: 1 }, "-=0.1");
      });
    }
    if (mm) mm.revert();
    mm = gsap.matchMedia();
